...
const CATEGORY_LABELS = {
  ...
  economy: 'Economia', army: 'Exército', world: 'Mundo'
};
  ...
  return <main class="cc-lobby">
    ...
    <section class="cc-server-section"><div class="cc-server-heading"><div><p class="cc-kicker">TRÊS MUNDOS PERSISTENTES</p><h2>Servidores disponíveis</h2></div><button onClick={onRefresh} disabled={loading}>↻ Atualizar</button></div>
      ...
      <div class="cc-server-grid">{servers.map(server => <article class={`cc-server-card ${server.phase}`} key={server.id}>
        ...
        <div class="cc-server-day"><strong>DIA {server.day}/{server.totalDays}</strong><span>{server.statusLabel}</span></div>
      ...
      <p class="cc-test-note">Modo de teste local: o relógio está acelerado para você verificar filas, economia e expansão sem esperar horas.</p>
    </section>
  </main>;
}
...
function RealmModal({ selected, regions, colors, religions, busy, error, onSelect, onSubmit }) {
  ...
  const [religion, setReligion] = useState(religions[0] || 'Cristianismo latino');
  ...
  return <div class="cc-modal-backdrop"><form class="cc-modal cc-parchment" onSubmit={event => { event.preventDefault(); onSubmit({ name, houseName, color, religion, regionId: selected?.id }); }}>
    <p class="cc-kicker">FUNDAÇÃO DA CASA</p><h1>Erga seu primeiro estandarte</h1><p>Dez coroas de IA já disputam este servidor. Sua escolha religiosa também influenciará a coesão do reino.</p>
    ...
    <label>Religião oficial<select value={religion} onChange={event => setReligion(event.currentTarget.value)}>{religions.map(item => <option key={item}>{item}</option>)}</select></label>
...
function RealmSection({ bootstrap, now, busy, onGoMap, onBuild, onRecruit, onCancel }) {
...
function Game({ serverId, onLeave }) {
  ...
  const common = { bootstrap, now, busy, onGoMap: goMap, onBuild: (regionId, type) => execute(() => crownsApi.queueBuilding(serverId, regionId, type)), onRecruit: regionId => execute(() => crownsApi.recruitArmy(serverId, regionId)), onCancel: actionId => execute(() => crownsApi.cancelAction(serverId, actionId)) };
  return <main class="cc-shell"><header class="cc-topbar"><button class="cc-brand cc-brand-button" onClick={onLeave}><span class="cc-crown">♔</span><span><small>SERVIDOR</small><strong>{serverId.replace('cc-world-', '0')}</strong></span></button><div class="cc-season"><small>{bootstrap.season.name}</small><strong>DIA {bootstrap.season.day}/{bootstrap.season.totalDays} · {bootstrap.season.statusLabel}</strong></div><div class="cc-resources"><span><small>OURO</small><b>{bootstrap.realm?.treasury ?? 0}</b></span><span><small>PROVISÕES</small><b>{bootstrap.realm?.provisions ?? 0}</b></span><span><small>PRESTÍGIO</small><b>{bootstrap.realm?.prestige ?? 0}</b></span></div><div class={`cc-online ${online ? 'connected' : ''}`}><i />{online ? 'Online' : 'Reconectando'}</div></header>
    ...
    {!bootstrap.realm && bootstrap.season.phase === 'open' ? <RealmModal selected={selected?.ownerRealmId || selected?.status !== 'neutral' ? null : selected} regions={bootstrap.regions} colors={bootstrap.customization.availableColors} religions={bootstrap.customization.religions} busy={busy} error={error} onSelect={setSelectedId} onSubmit={payload => execute(() => crownsApi.createRealm(serverId, payload))} /> : null}
