...
async function boot(): Promise<void> {
  try {
    ...
    hud = new HudController(store, {
      ...
      advanceStage: () => runAction(() => api.advanceStage(), 'A congregação alcançou um novo estágio!'),
      claimDaily: () => runAction(() => api.claimDaily(), 'Check-in diário recebido.'),
      claimMission: (missionId) => runAction(() => api.claimMission(missionId), 'Recompensa da missão recebida.'),
      claimWeekly: () => runAction(() => api.claimWeekly(), 'Recompensa semanal recebida.'),
