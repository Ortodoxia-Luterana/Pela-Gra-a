...
async function attendReligion(state) {
  for (const movement of state.religiousMovements || []) {
    if (movement.relevant && !movement.response && !religiousDecisions.has(movement.id)) {
...
(async () => {
  try {
    ...
    await api('/realm/create', { serverId: 'cc-world-1', name: 'Reino da Campanha Total', houseName: 'Casa Veritas', religion: 'Cristianismo', color: state.customization.availableColors[0], regionId: capital.id });
    state = await bootstrap();
    assert.equal(state.season.phase, 'open');
    assert.equal(state.realm.religion, 'Cristianismo');
    assert.ok(new Set(state.realm.court.diplomacy.knownRealms.map(realm => realm.religion)).size >= 2);
    ...
    await api('/diplomacy/gift', { serverId: 'cc-world-1', targetRealmId: sameFaith.id, resourceType: 'wood', amount: 100 });
    state = await waitUntil(item => item.season.day >= 2 && item.diplomaticRequests.length >= 1, Math.max(8_000, gameDayMs * 4), 'primeiro pedido diplomático');
    const request = state.diplomaticRequests[0];
    await api('/diplomacy/request/respond', { serverId: 'cc-world-1', requestId: request.id, accept: true });
    state = await bootstrap();
    ...
    await waitActions();
    state = await waitUntil(item => item.season.day >= 10 && item.religiousMovements.some(movement => movement.relevant), Math.max(12_000, gameDayMs * 12), 'primeiro movimento religioso');
    const relevantMovement = state.religiousMovements.find(movement => movement.relevant);
    qaDb.prepare("UPDATE cc_region_religions SET heresy_share = 35, heresy_name = ? WHERE season_id = 'cc-world-1' AND region_id = ?").run(relevantMovement.name, state.realm.capitalRegionId);
